...
dirc = "/mnt/ufs18/home-033/gangulia/Variable_selection_Lassonet/tractography_data/"
X, Y, X_train, Y_train, X_test, Y_test, supp_true, grp_length, g_unlist, active_set = data_load_new(directory=dirc)
...
def fdr(i):
    ...
    for index in range(i):
        c=[]
        for b in range(B):
            if (results_ordered[b][index]<(ordered_imp[index]-40) or results_ordered[b][index]>(ordered_imp[index]+40)): c.append(1)
            else: c.append(0)
...
def pred_bootstrap(iter):
    ...
    history=model.fit(Xtrain_scale_bt[:,reduced_sel_pos], Ytrain_scale_bt, epochs=30, batch_size=50, verbose=0, validation_split=0.2)
...
after_error = np.mean(pred_results)   
...
def pred_bootstrap(iter):
    ...
    model.compile(loss='mse', optimizer='adam', metrics=['mse','mae'])
    history=model.fit(Xtrain_scale_bt, Ytrain_scale_bt, epochs=30, batch_size=50, verbose=0, validation_split=0.2)
